...
/**
 * 2026-08 の LIVE 観測で実在した形（779 件中 2 件）。
 * 明細も決済も金額も持たず、returns / refunds / net_amounts だけを持つ。
 */
function returnContainer() {
  return {
    ...
    location_id: "L1",
    state: "COMPLETED",
    created_at: "2026-08-08T11:56:57.009Z",
    closed_at: "2026-08-08T11:56:58.545Z",
    updated_at: "2026-08-08T11:56:58.545Z",
    version: 2,
    source: {},
    returns: [{}],
    refunds: [{}],
...
test("return_container_excluded: 観測された返品コンテナは集計から外れる", async () => {
  const f = fakeFetch([
    { json: { orders: [...orders(2, { amount: 1100, tax: 100 }), returnContainer()] } },
  ]);
  const r = await fetchMtd({ ...base, fetchImpl: f.impl });
  assert.equal(r.error_state.state, "OK", "返品 1 件で月全体を落とさない");
  assert.equal(r.excluded_return_containers, 1);
  assert.equal(r.order_count, 2, "返品は order_count に入れない");
  assert.equal(r.sales_tax_inclusive, 2200, "売上額は返品の影響を受けない");
  assert.equal(r.sales_tax_exclusive, 2000);
});

test("return_container_excluded: 0 円の売上として数えない", async () => {
  const f = fakeFetch([{ json: { orders: [returnContainer()] } }]);
  const r = await fetchMtd({ ...base, fetchImpl: f.impl });
  assert.equal(r.error_state.state, "OK");
  assert.equal(r.order_count, 0, "返品だけなら売上は 0 件");
  assert.equal(r.excluded_return_containers, 1, "0 件と 1 件除外は別の事実");
});

test("excluded_return_containers: 該当なしでも 0 を明示する", async () => {
  const f = fakeFetch([{ json: { orders: orders(3) } }]);
  const r = await fetchMtd({ ...base, fetchImpl: f.impl });
  assert.ok("excluded_return_containers" in r, "field を落とさない");
  assert.equal(r.excluded_return_containers, 0);
});

test("excluded_return_containers: CONFIG_MISSING でも field は揃う", async () => {
  const f = fakeFetch([]);
  const r = await fetchMtd({ ...base, accessToken: undefined, fetchImpl: f.impl });
  assert.equal(r.error_state.state, "CONFIG_MISSING");
  assert.equal(r.excluded_return_containers, 0);
...
/** 7 条件のうち 1 つでも外れれば、除外ではなく fail-closed に落ちる。 */
const NOT_RETURN_CONTAINERS = [
  ["line_items を持つ", { ...returnContainer(), line_items: [{}] }],
  ["tenders を持つ", { ...returnContainer(), tenders: [{}] }],
  ["returns が無い", (() => { const o = returnContainer(); delete o.returns; return o; })()],
  ["refunds が無い", (() => { const o = returnContainer(); delete o.refunds; return o; })()],
  ["net_amounts が無い", (() => { const o = returnContainer(); delete o.net_amounts; return o; })()],
  ["total_tax_money だけ持つ", { ...returnContainer(), total_tax_money: { amount: 100 } }],
];

for (const [label, order] of NOT_RETURN_CONTAINERS) {
  test(`unknown_missing_money_shape: ${label} は除外せず MALFORMED_RESPONSE`, async () => {
    const f = fakeFetch([{ json: { orders: [order] } }]);
    const r = await fetchMtd({ ...base, fetchImpl: f.impl });
    assert.equal(r.error_state.state, "MALFORMED_RESPONSE", "除外の範囲を広げない");
    assert.equal(r.sales_tax_inclusive, null);
    assert.equal(r.excluded_return_containers, 0, "未知の形を除外件数に混ぜない");
  });
}

test("return_container_excluded: total_money を持つ返品類似は売上として扱う", async () => {
  const withMoney = {
    ...returnContainer(),
    total_money: { amount: 500 },
    total_tax_money: { amount: 50 },
  };
  const f = fakeFetch([{ json: { orders: [withMoney] } }]);
  const r = await fetchMtd({ ...base, fetchImpl: f.impl });
  assert.equal(r.error_state.state, "OK");
  assert.equal(r.excluded_return_containers, 0);
  assert.equal(r.order_count, 1);
  assert.equal(r.sales_tax_inclusive, 500);
});

test("pagination: 返品コンテナがページを跨いでも cursor を尽くし、除外を合算する", async () => {
  const f = fakeFetch([
    { json: { orders: [...orders(500), returnContainer()], cursor: "c1" } },
    { json: { orders: [...orders(278), returnContainer()] } },
  ]);
  const r = await fetchMtd({ ...base, fetchImpl: f.impl });
  assert.equal(r.error_state.state, "OK");
  assert.equal(r.pagination_pages, 2, "500 件で打ち切らない");
  assert.equal(r.order_count, 778);
  assert.equal(r.excluded_return_containers, 2, "ページを跨いで足し合わせる");
});

test("freshness fields が常に揃う", async () => {
  const f = fakeFetch([{ json: { orders: orders(1) } }]);
  const r = await fetchMtd({ ...base, fetchImpl: f.impl });
  for (const key of [
    "fetched_at",
    "period_start",
  ...
  assert.equal(r.period_start, "2026-08-01T03:00:00+09:00");
});
